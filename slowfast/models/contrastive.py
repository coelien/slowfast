...
from slowfast.utils import AllGatherWithGradient

import slowfast.models.losses as losses
# import slowfast.utils.distributed as du
import slowfast.logging as logging
from slowfast.models.slowfast import ResNet, SlowFast

from .build import MODEL_REGISTRY
from ..utils import cat_all_gather, get_local_rank, get_world_size, get_rank, get_local_size, all_reduce, all_gather
...
class ContrastiveModel(nn.Module):
    ...
    def _batch_shuffle(self, x):
        ...
        if self.num_gpus > 1:
            if self.cfg.CONTRASTIVE.LOCAL_SHUFFLE_BN:
                x = cat_all_gather(x, local=True)
                if another_crop:
                    x_crop = cat_all_gather(x_crop, local=True)
                world_size = get_local_size()
                gpu_idx = get_local_rank()
            else:
                x = cat_all_gather(x)
                if another_crop:
                    x_crop = cat_all_gather(x_crop)
        ...
        if self.type == "mem":
            ...
            if distributed_loss and self.cfg.NUM_GPUS > 1:
                ...
                if cat_across_gpus and self.cfg.NUM_GPUS > 1:
                    q = AllGatherWithGradient.apply(q)
                    q2 = AllGatherWithGradient.apply(q2)
                out = torch.cat([q, q2], dim=0)
                # [2*B, 2*B]
    ...
    def distributed_sinkhorn(self, Q, nmb_iters):
        with torch.no_grad():
            sum_Q = torch.sum(Q)
            all_reduce([sum_Q], average=False)
            Q /= sum_Q

            u = torch.zeros(Q.shape[0]).cuda(non_blocking=True)
            r = torch.ones(Q.shape[0]).cuda(non_blocking=True) / Q.shape[0]
            c = torch.ones(Q.shape[1]).cuda(non_blocking=True) / (
                get_world_size() * Q.shape[1]
            )

            curr_sum = torch.sum(Q, dim=1)
            all_reduce([curr_sum], average=False)
...
class Memory(nn.Module):
    ...
    def update(self, mem, momentum, ind, time, interp=False):
        if self.num_gpus > 1:
            mem, ind, time = all_gather([mem, ind, time])
        with torch.no_grad():
            if interp:
                t0 = time.floor().long()  # - 1
                t0 = torch.clamp(t0, 0, self.memory.shape[1] - 1)
