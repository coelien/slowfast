...
from slowfast import misc, metrics, logging
from slowfast.models import losses
from slowfast.models.build import build_model
from slowfast.datasets import loader
from slowfast.datasets.mixup import MixUp
from slowfast.logging import get_logger
from slowfast.meters import TrainMeter, ValMeter, EpochTimer
from slowfast.multigrid import MultigridSchedule
import slowfast.visualization.tensorboard_vis as tb
from slowfast.utils import all_reduce, all_gather_unaligned, init_distributed_training, is_master_proc, all_gather
import slowfast.optimizer as optim
import slowfast.checkpoint as cu
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                # Gather all the predictions across all the devices.
                if cfg.NUM_GPUS > 1:
                    loss, top1_err, top5_err = all_reduce(
...
def eval_epoch(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.NUM_GPUS > 1:
                preds = torch.cat(all_gather_unaligned(preds), dim=0)
                ori_boxes = torch.cat(all_gather_unaligned(ori_boxes), dim=0)
                metadata = torch.cat(all_gather_unaligned(metadata), dim=0)
            ...
            if cfg.DATA.MULTI_LABEL:
                if cfg.NUM_GPUS > 1:
                    preds, labels = all_gather([preds, labels])
            else:
                # Compute the errors.
                num_topks_correct = metrics.topks_correct(preds, labels, (1, 5))
                ...
                if cfg.NUM_GPUS > 1:
                    top1_err, top5_err = all_reduce([top1_err, top5_err])
...
def build_trainer(cfg):
    ...
    if is_master_proc() and cfg.LOG_MODEL_INFO:
...
def train(cfg):
    ...
    if cfg.TENSORBOARD.ENABLE and is_master_proc(
